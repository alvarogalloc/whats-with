...
questionItemHeaders.forEach((questionItemHeader) => {
  questionItemHeader.addEventListener("click", (event) => {
    const currentlyActivequestionItemHeader = document.querySelector(
      ".question-item-header.active"
    );
    if (
      currentlyActivequestionItemHeader &&
      currentlyActivequestionItemHeader !== questionItemHeader
    ) {
